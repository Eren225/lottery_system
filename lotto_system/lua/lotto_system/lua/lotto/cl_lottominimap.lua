...
local tempsFermetureInscription = ErenConfig.closeTime // temps avant fermeture des inscriptions
...
local function minimap(status)
    if status then
        ...
        DermaPanel:SetSize(RespW(450), RespH(200)) 
        ...
        DermaPanel.Paint = function(self, w, h)
            ...
            draw.SimpleText("Une loterie est en cours !", "Font_small",w/20, h/2.6, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText("Le coût de participation est de "..ErenConfig.joinCost.."€" , "Font_small",w/20, h/1.9, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
