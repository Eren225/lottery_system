...
local participants = ""
local cooldownLotto = ErenConfig.cooldownLotto + ErenConfig.tempsChoixGagnant// cooldown + temps choix gagnant
...
local function lottoPanel(open)
    if open then
        PanelConfig = vgui.Create("DFrame")
        PanelConfig:SetSize(RespW(1000), RespH(700))
        PanelConfig:Center()
        PanelConfig:SetTitle("")
        PanelConfig:SetDraggable(false)
...
hook.Add( "OnPlayerChat", "HelloCommand", function( ply, strText, bTeam, bDead ) 
    if ( ply != LocalPlayer()|| string.Trim(string.lower(strText))!="!loterie" ||! ErenConfig.leaderTeam[ply:Team()]) then return end 
    lottoPanel(true)
end )